...
final class ControlSceneDelegate: UIResponder, UIWindowSceneDelegate {
    var window: UIWindow?
    private var externalMirrorManager: RebootExternalMirrorManager?
    ...
    func scene(
        ...
        let manager = RebootExternalMirrorManager(model: rootViewController.appModel)
        manager.start()
        externalMirrorManager = manager
    }
    ...
    func sceneDidDisconnect(_ scene: UIScene) {
        externalMirrorManager?.stop()
        externalMirrorManager = nil
    }
}

@MainActor
private final class RebootExternalMirrorManager {
    ...
    func start() {
        guard screenObservers.isEmpty else {
            syncExternalWindow()
            return
        }
    ...
    private func syncExternalWindow() {
        guard let externalScreen = availableExternalScreen() else {
            tearDownExternalWindow()
            return
        }
        ...
        let mirrorController = RebootExternalMirrorViewController(model: model)
        let externalWindow = UIWindow(frame: externalScreen.bounds)
        externalWindow.screen = externalScreen
        externalWindow.rootViewController = mirrorController
        externalWindow.windowLevel = .normal + 1
        externalWindow.isHidden = false
        self.externalWindow = externalWindow
    }

    private func tearDownExternalWindow() {
        guard let externalWindow else {
            return
        }
    ...
    private func availableExternalScreen() -> UIScreen? {
...
private final class RebootExternalMirrorViewController: UIViewController {
    ...
    private func render(_ state: TerminalSurfaceState) {
        ...
        if state.transcript.isEmpty {
            outputView.text = state.statusMessage
        } else {
