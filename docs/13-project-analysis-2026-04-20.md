...
## 10) Addendum (2026-04-21)

Реализованы ключевые улучшения интерфейса и управляемости для админского сценария:

- Введена централизованная система визуальных токенов (`AdminTheme`) и runtime-переключение темы через `Profile`.
- Выбран компактный modern-dark подход для рабочих экранов с сохранением плотности информации:
  - акцент на читаемость данных, статусов и вводимых команд,
  - контрастные, но не агрессивные цвета действий и ошибок.
- Темы применены к основным пользовательским формам:
  - карточка хоста,
  - формы создания/редактирования хоста,
  - password prompt,
  - SSH session form.
- Добавлен первый рабочий срез App Intents:
  - `OpenWorkspaceIntent`,
  - `ConnectSavedHostIntent`,
  - App Shortcuts provider с обновлением параметров запуска.

Итог: приложение визуально стало более целостным и современным, при этом UI не снижает функциональную скорость для продвинутых пользователей и администраторов.
